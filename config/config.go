...
var (
	prefix        = "cheat"
	files         = []string{"cheat.config.development.local", "cheat.config.development", "cheat.config.local", "cheat.config"}
	absoluteFiles = []string{"/etc/cheat/server.config"}
...
func Get() (Config, []FutureLog) {
	...
	if config.AuthMode != AuthModeTurn && config.AuthMode != AuthModeAll && config.AuthMode != AuthModeNone {
		logs = append(logs,
			futureFatal(fmt.Sprintf("invalid CHEAT_AUTH_MODE: %s", config.AuthMode)))
	}

	if config.ExternalIP == "" {
		logs = append(logs, futureFatal("CHEAT_EXTERNAL_IP must be set"))
	}

	if config.ServerTLS {
		if config.TLSCertFile == "" {
			logs = append(logs, futureFatal("CHEAT_TLS_CERT_FILE must be set if TLS is enabled"))
		}

		if config.TLSKeyFile == "" {
			logs = append(logs, futureFatal("CHEAT_TLS_KEY_FILE must be set if TLS is enabled"))
		}
	}
	...
	if len(config.Secret) == 0 {
		...
		if _, err := rand.Read(config.Secret); err == nil {
			logs = append(logs, FutureLog{
				...
				Msg:   "CHEAT_SECRET unset, user logins will be invalidated on restart"})
		} else {
	...
	if net.ParseIP(config.ExternalIP) == nil || config.ExternalIP == "0.0.0.0" {
		logs = append(logs, futureFatal(fmt.Sprintf("invalid CHEAT_EXTERNAL_IP: %s", config.ExternalIP)))
	}

	min, max, err := config.parsePortRange()
	if err != nil {
		logs = append(logs, futureFatal(fmt.Sprintf("invalid CHEAT_TURN_PORT_RANGE: %s", err)))
	} else if min == 0 && max == 0 {
		// valid; no port range
	} else if min == 0 || max == 0 {
		logs = append(logs, futureFatal("invalid CHEAT_TURN_PORT_RANGE: min or max port is 0"))
	} else if min > max {
		logs = append(logs, futureFatal(fmt.Sprintf("invalid CHEAT_TURN_PORT_RANGE: min port (%d) is higher than max port (%d)", min, max)))
	} else if (max - min) < 40 {
		logs = append(logs, FutureLog{
			Level: zerolog.WarnLevel,
...
func getExecutableOrWorkDir() (string, *FutureLog) {
	...
	// when using `go run serve.go` the executable lives in th temp directory therefore the env.development
...
func getFiles(relativeTo string) []string {
	...
	if err == nil {
		result = append(result, filepath.Join(homeDir, ".config/cheat/server.config"))
	}
